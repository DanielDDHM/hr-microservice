...
import { createContactValidation } from '~/validations';
import genericService from '../generic';
import prisma from '~/services/prisma';

const createContactService = async (dto: ContactDTO) => {
    try {
        const {
            label,
            value,
            employeeId
        } = createContactValidation.parse(dto);
        ...
        const contact = await genericService.create({
            label,
            value,
            employeeId
        }, 'contact');
